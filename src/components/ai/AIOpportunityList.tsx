...
function OpportunityEvidence({ selected }: { selected: AiWorkGraphDrilldownRef | null }) {
  ...
  if (error) {
    return (
      <p className="mt-3 rounded-2xl border border-(--accent-negative)/30 bg-red-500/5 px-3 py-2 text-xs text-red-600">
          ...
          {item.workGraphDrilldowns.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-2">
              {item.workGraphDrilldowns.map((ref) => {
                const selected =
                  selectedRef?.rootType === ref.rootType && selectedRef.rootId === ref.rootId;
                return (
                  <button
          ...
          <OpportunityEvidence
            selected={
              ...
              item.workGraphDrilldowns.some(
                (ref) => ref.rootType === selectedRef.rootType && ref.rootId === selectedRef.rootId,
              )
                ? selectedRef
                : null
            }

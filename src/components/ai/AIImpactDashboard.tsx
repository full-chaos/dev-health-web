...
export function AIImpactDashboard({ filter }: AIImpactDashboardProps) {
  ...
  if (summary && !summary.dataAvailable) {
    return (
      <AIEmptyState title="AI workflow data has not populated yet">
        Connect a GitHub provider to populate AI-assisted PR attribution, review signals, test gaps, and incident linkage.
      </AIEmptyState>
    );
  }
  ...
  const leverage = bucketRows.find((row) => row.bucket === "AI_ASSISTED")?.leverage ?? bucketRows[0]?.leverage;
    ...
    <div className="flex flex-col gap-6" data-testid="ai-impact-dashboard">
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <div className="rounded-3xl border border-(--card-stroke) bg-card p-5">
          <p className="text-xs uppercase tracking-[0.15em] text-(--ink-muted)">AI-assisted work share</p>
          <p className="mt-2 text-4xl font-semibold tabular-nums">{formatPercent(summary?.aiAssistedPrRatio)}</p>
          <p className="mt-2 text-sm text-(--ink-muted)">{summary?.aiAssistedPrs ?? 0} of {summary?.totalPrs ?? 0} PRs lean AI-assisted.</p>
        </div>
        <div className="rounded-3xl border border-(--card-stroke) bg-card p-5">
        ...
        <div className="rounded-3xl border border-(--card-stroke) bg-card p-5">
          ...
          <p className="mt-2 text-sm text-(--ink-muted)">Kept visible so data coverage gaps stay inspectable.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        ...
        <AIPanelCard title="Top affected repos and teams" description="Scoped rollups will appear after repo/team AI summary coverage lands.">
          <AIEmptyState title="Coming after detector lands">
            Org-scoped AI impact data populates now. Repo and team ranking placeholders remain empty to avoid fabricating scoped values.
          </AIEmptyState>
        </AIPanelCard>
      ...
      <p className="text-xs text-(--ink-muted)">Last computed {summary?.computedAt ?? "not yet available"}. Copy uses system-health language: values suggest patterns and should be interpreted with local context.</p>
    </div>
  );
}

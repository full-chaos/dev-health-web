...
import {
	useAIComparison,
	useAIImpactSummary,
} from "@/lib/graphql/hooks/useAIImpact";
...
export function AIImpactDashboard({ filter }: AIImpactDashboardProps) {
	...
	const leverage =
		bucketRows.find((row) => row.bucket === "AI_ASSISTED")?.leverage ??
		...
		<div className="flex flex-col gap-6" data-testid="ai-impact-dashboard">
			<div className="grid grid-cols-1 gap-4 md:grid-cols-3">
				...
				<div className="rounded-3xl border border-(--card-stroke) bg-card p-5">
					...
					<p className="mt-2 text-sm text-(--ink-muted)">
						{formatPercent(
							safeRatio(summary?.agentCreatedPrs, summary?.totalPrs),
						)}{" "}
						of PRs appear agent-created.
					</p>
				</div>
				<div className="rounded-3xl border border-(--card-stroke) bg-card p-5">
					<p className="text-xs uppercase tracking-[0.15em] text-(--ink-muted)">
			...
			<div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
					...
					<div className="flex items-end justify-between gap-4">
						<div>
							<p className="text-4xl font-semibold tabular-nums">
								{agentBucket?.agentCreatedPrCount ??
									summary?.agentCreatedPrs ??
									0}
							</p>
							<p className="text-sm text-(--ink-muted)">agent-created PRs</p>
						</div>
						<p className="text-sm text-(--ink-muted)">
							{bucketLabel("AGENT_CREATED")}
						</p>
					</div>
					{trend.length ? (
				...
				>
					<div className="flex flex-col gap-3 text-sm text-(--ink-muted)">
						<p>
							Automation candidates moved out of the Impact dashboard so
							leverage diagnostics and candidate triage can evolve
							independently.
						</p>
						<Link
			...
			<p className="text-xs text-(--ink-muted)">
				Last computed {summary?.computedAt ?? "not yet available"}. Copy uses
				system-health language: values suggest patterns and should be
				interpreted with local context.
			</p>
		</div>
	);
}
	...
	return (
		<div className="grid gap-4 md:grid-cols-3" data-testid="ai-impact-loading">
			{Array.from({ length: 6 }).map((_, index) => (
				<div
					key={index}
					className="h-40 animate-pulse rounded-3xl bg-(--card-80)"
				/>
			))}
		</div>
	);

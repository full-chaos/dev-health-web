...
vi.mock("@/components/charts/DonutChart", () => ({
    DonutChart: ({ data }: { data: Array<{ name: string; value: number }> }) => (
        <div data-testid="donut-chart">{data.map((item) => item.name).join(",")}</div>
    ),
}));

vi.mock("@/components/charts/TimeseriesChart", () => ({
    TimeseriesChart: ({ data }: { data: Array<{ day: string; value: number }> }) => (
        <div data-testid="timeseries-chart">{data.length}</div>
    ),
}));
...
vi.mock("@/components/charts/SparklineChart", () => ({
    SparklineChart: ({ data }: { data: number[] }) => (
        <div data-testid="sparkline-chart">{data.join(",")}</div>
    ),
}));
...
function mockDefaults() {
    mockUseAIOpportunities.mockReturnValue({
        data: { aiOpportunities: { orgId: "org", detectorReady: false, recommendations: [] } },
        fetching: false,
        error: undefined,
    });
        ...
        data: {
            aiComparison: {
                ...
                baselineSide: {
                    ...
                    revertRate: 0.02,
                    incidentRate: 0.01,
                    cycleTimeAvgHours: 24,
                },
                delta: {
                    reviewsPerPrDelta: 0.6,
                    reworkRateDelta: 0.1,
                    testGapRateDelta: 0.05,
                    revertRateDelta: 0.03,
                    incidentRateDelta: 0.01,
                    cycleTimeDeltaHours: -4,
                },
            },
        },
        fetching: false,
                    ...
                    byBucket: [
                        {
                            ...
                            cycleTimeAvgHours: 20,
                            aiCycleTimeDeltaHours: -4,
                            aiReviewAmplification: 0.5,
                            reworkDragRate: 0.1,
                            revertRate: 0.02,
                        ...
                        },
                        {
                            ...
                            agentCreatedPrCount: 3,
                            cycleTimeAvgHours: 18,
                            aiCycleTimeDeltaHours: -6,
                            aiReviewAmplification: 0.8,
                            reworkDragRate: 0.2,
                            revertRate: 0.03,
                            incidentDragRate: 0.02,
                            testGapRate: 0.08,
                            leverage: {
                                prsComponent: 1,
                                cycleTimeComponent: 1,

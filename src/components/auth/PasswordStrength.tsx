          ...
          return (
            <div key={check.label} className="flex items-center gap-2 text-sm">
              {passed ? (
                ...
                <svg className="h-4 w-4 text-[var(--ink-muted)] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                </svg>
              )}
              <span className={passed ? "text-[var(--foreground)]" : "text-[var(--ink-muted)]"}>
                {check.label}
              </span>
            </div>
          )

  ...
  return (
    <div className="rounded-3xl border border-(--card-stroke) bg-card p-6 space-y-3 animate-pulse">
      ...
      {Array.from({ length: lines }, (_, i) => (
        <div key={`skeleton-line-${i}`} className={`h-4 bg-(--card-70) rounded ${i % 2 === 0 ? "w-2/3" : "w-1/2"}`} />
      ))}
    </div>
  );
}
  ...
  return (
    <div className="space-y-2 animate-pulse">
      ...
      {Array.from({ length: rows }, (_, i) => (
        <div key={`skeleton-row-${i}`} className="grid gap-4" style={{ gridTemplateColumns: `repeat(${cols}, 1fr)` }}>
          {Array.from({ length: cols }, (_, j) => (
            <div key={`skeleton-cell-${i}-${j}`} className="h-4 bg-(--card-70) rounded" />
          ))}
        </div>
      ))}
    </div>
...
export function SkeletonChart({ height = "h-64" }: { height?: string }) {
  return (
    <div className={`${height} rounded-3xl border border-(--card-stroke) bg-card p-6 animate-pulse`}>

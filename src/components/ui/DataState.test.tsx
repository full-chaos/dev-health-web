...
const EMPTY_VARIANT_TITLES: Record<Exclude<DataStateVariant, "loading" | "error">, string> = {
...
describe("DataState", () => {
    it.each(Object.entries(EMPTY_VARIANT_TITLES) as [DataStateVariant, string][])(
        "renders controlled taxonomy copy for the %s variant",
        (variant, title) => {
    ...
    it("renders an error state that is visually distinct from empty states", () => {
        ...
        expect(container.querySelector(".border-\\(--accent-negative\\)\\/30")).not.toBeNull();
    ...
    it("honors a custom data-testid", () => {
        render(<DataState variant="insufficient-confidence" data-testid="panel-empty" />);

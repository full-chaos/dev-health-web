			...
			const fetchData = async () => {
				try {
					...
					if (apiUrl) {
						...
						if (!res.ok)
							throw new Error(ValidationErrors.FailedToFetchEvidence);
					...
					if (result) {
						...
						const magnitude =
							Math.abs(deltaPct) > 10 ? "Significant" : "Moderate";
						...
						const actions =
							result.actions || definition?.suggestedActions || [];
						...
						<>
							...
							{data.evidence?.length ? (
								...
								<div className="rounded-2xl border border-dashed border-(--card-stroke) bg-(--card-90) p-4 text-sm leading-6 text-(--ink-muted)">
									...
									filter window. This is a partial-data state, not a zero
									signal.
								</div>
							)}
...
function EvidenceProvenanceStrip({
	provenance,
}: {
	provenance?: EvidenceProvenance;
}) {
	...
	return (
		<section className="grid gap-3 rounded-2xl border border-(--card-stroke) bg-(--card-90) p-4 text-xs text-(--ink-muted)">
			...
			<div className="grid gap-2 sm:grid-cols-2">
				...
				<EvidenceProvenanceItem
					label="Quality"
					value={provenance?.quality || "partial"}
				/>
				<EvidenceProvenanceItem
					label="Last sync"
			...
			{provenance?.partial && (
				<p className="rounded-xl bg-amber-500/10 px-3 py-2 text-amber-300">
					Partial evidence: the backend did not return a complete artifact list
					for this selection.
				</p>
			)}
		</section>

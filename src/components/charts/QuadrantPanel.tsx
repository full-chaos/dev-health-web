...
export function QuadrantPanel({
	...
	const [selectedPoint, setSelectedPoint] = useState<QuadrantPoint | null>(
		null,
	);
	const [selectedPointKey, setSelectedPointKey] = useState<string | null>(null);
	const [showZoneOverlay, setShowZoneOverlay] = useState(true);
	const [hoveredOverlayKey, setHoveredOverlayKey] = useState<string | null>(
		null,
	);
	...
	const axesKey = scopedData
		? `${scopedData.axes.x.metric}:${scopedData.axes.y.metric}`
		: null;
	...
	const influenceFraming =
		influenceNarrative?.framing ?? "Operating modes under paired pressures.";
	const influenceNext =
							...
							<div className="min-w-0 rounded-2xl border border-(--card-stroke) bg-(--card-80) p-4 text-xs text-(--ink-muted)">
								...
								<div className="mt-3 space-y-3">
									{zoneLegendItems.map((item) => {
										const isActive =
											activeHoveredOverlayKey === item.overlayKey;
										return (
											<button
												key={item.key}
												type="button"
												onMouseEnter={() =>
													setHoveredOverlayKey(item.overlayKey)
												}
												onMouseLeave={() => setHoveredOverlayKey(null)}
												onFocus={() => setHoveredOverlayKey(item.overlayKey)}
												onBlur={() => setHoveredOverlayKey(null)}
												onClick={() => setHoveredOverlayKey(item.overlayKey)}
											...
											>
												...
												<div className="min-w-0">
													...
													{isActive && (
														<p className="mt-1 break-words text-xs leading-snug text-(--ink-muted) animate-in fade-in slide-in-from-top-1 duration-200">
															{item.description}
														</p>
													)}
												</div>

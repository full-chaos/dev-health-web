  ...
  return (
    <div className="flex h-full flex-col bg-(--card-80) text-xs shadow-xl animate-in fade-in slide-in-from-right-4 duration-300">
      <header className="flex items-center justify-between border-b border-(--card-stroke) p-4">
        <div>
          <p className="text-[10px] uppercase tracking-widest text-(--ink-muted)">Investigation</p>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        <section>
          <div className="flex items-center justify-between mb-2">
            <p className="text-[10px] uppercase tracking-[0.2em] text-(--ink-muted)">Summary</p>
          ...
          <p className="text-[13px] leading-relaxed text-foreground">
            <span className="font-semibold text-(--accent-2)">{roleConfig.framing}</span> Observed
            state for <span className="font-semibold">{point.entity_label}</span> during the window
            of {point.window_start} to {point.window_end}.
          </p>
        </section>

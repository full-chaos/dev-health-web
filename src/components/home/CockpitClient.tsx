...
const deltaAccent = (value: number) =>
  value > 0 ? "text-(--accent-3)" : value < 0 ? "text-(--accent-negative)" : "text-(--ink-muted)";
    ...
    <>
      ...
      <section>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs uppercase tracking-[0.15em] text-(--ink-muted)">Key shifts</p>
            <p className="mt-1 text-sm text-(--ink-muted)">Small shifts in the selected window.</p>
          </div>
          <Link
      ...
      <section className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <div className="rounded-3xl border border-(--card-stroke) bg-(--card-80) p-5">
          ...
          <p className="mt-2 text-sm text-(--ink-muted)">Short shifts from the selected window.</p>
        ...
        <div className="rounded-3xl border border-(--card-stroke) bg-(--card-80) p-5">
          ...
          <div className="mt-4 grid gap-3">
            {home?.tiles
              ? Object.entries(home.tiles).map(([key, tile]) => (
                  ...
                  >
                    ...
                    <p className="mt-2 text-base font-semibold text-foreground">{tile.subtitle}</p>
            ...
            >
              <p className="text-xs uppercase tracking-[0.2em] text-(--ink-muted)">Focus thread</p>

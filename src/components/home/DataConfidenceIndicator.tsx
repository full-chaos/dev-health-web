...
> = {
	...
	medium: {
		...
		blurb:
			"Some sources are missing or sparse — read trends, not point values.",
	},
	low: {
...
export function DataConfidenceIndicator({
	...
	const { level, coverage_pct, connected_sources, missing_sources, caveats } =
		confidence;
	const meta = LEVEL_META[level] ?? LEVEL_META.low;
	const hasCoverage =
		typeof coverage_pct === "number" && Number.isFinite(coverage_pct);

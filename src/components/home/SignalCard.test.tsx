...
describe("SignalCard", () => {
	it("renders all four required encodings plus why + recommended action", () => {
		...
		expect(screen.getByTestId("signal-scope")).toHaveTextContent(
			"3 repos · payments",
		);
		expect(screen.getByTestId("signal-evidence-count")).toHaveTextContent(
			/7 artifacts/i,
		);

		// why + recommended action
		expect(screen.getByTestId("signal-why")).toHaveTextContent(
			/longer reviews delay/i,
		);
	...
	it.each([
		...
		"medium",
		"low",
	] as const)("encodes %s severity on the card", (severity) => {
		render(
			<SignalCard
	...
	it.each([
		...
		"medium",
		"low",
	] as const)("encodes %s confidence on the card", (confidence) => {
		render(
			<SignalCard

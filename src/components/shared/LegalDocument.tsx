  ...
  return (
    <section className="mx-auto max-w-4xl px-6 pb-24 pt-16 sm:pt-24">
      <div className="rounded-[2rem] border border-(--card-stroke) bg-(--card-80) p-8 shadow-[0_20px_60px_-40px_rgba(0,0,0,0.45)] sm:p-12">
        ...
        <h1 className="mt-4 font-(--font-display) text-4xl leading-tight sm:text-5xl">{title}</h1>
          ...
          {sections.map((section) => (
            <section key={section.title} className="space-y-4">
              ...
              {section.paragraphs.map((paragraph) => (
                <p key={paragraph} className="text-sm leading-7 text-(--ink-muted) sm:text-base">
                  {paragraph}
                </p>
              ))}
              {section.items ? (

          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-(--card-stroke)">
              {invoice.line_items.map((line) => (
                <tr key={line.id}>
                  <td className="px-3 py-3 text-foreground">{line.description ?? "Line item"}</td>
                  <td className="px-3 py-3 text-(--ink-muted)">{line.quantity}</td>
                  <td className="px-3 py-3 text-(--ink-muted)">{line.stripe_price_id ?? "-"}</td>

...
describe("RepoSelector", () => {
  ...
  describe("error state", () => {
    it("shows error message when fetch fails", async () => {
      ...
      renderSelector();
      await waitFor(() => {
        expect(screen.getByText(/Failed to load repositories: Unauthorized/i)).toBeInTheDocument();
      });
    });
  });
  ...
  describe("with repos loaded", () => {
    beforeEach(() => {
      mockListRepos.mockResolvedValue({
        data: { provider: "github", owner: "myorg", repos: MOCK_REPOS, total: MOCK_REPOS.length },
      });
    });
    ...
    it("Select All selects all repos", async () => {
      ...
      expect(onSelectionChange).toHaveBeenCalledWith(["repo-alpha", "repo-beta", "repo-gamma"]);
    });

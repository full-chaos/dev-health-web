      ...
      <div className="w-full max-w-2xl rounded-xl border border-(--card-stroke) bg-(--card) shadow-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              {PROVIDERS.map((provider) => (
                ...
                >
                  ...
                  <span className="text-sm text-(--ink-muted)">{provider.description}</span>
                </button>
              ))}
            </div>
          )}
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <select
                    value={onConflict}
                    onChange={(e) => setOnConflict(e.target.value as "skip" | "merge")}
                      ...
                      {discoveredTeams.map((team) => (
                        ...
                        >
                          <td className="p-3">
                            <input
                              type="checkbox"
                              checked={selectedTeams.has(team.provider_team_id)}
                              onChange={() => toggleTeam(team.provider_team_id)}
                          ...
                          <td className="p-3 text-(--ink-muted)">
                            {Object.entries(team.associations).map(([key, val]) => (
          ...
          {step === "importing" && (
            <div className="flex flex-col items-center justify-center py-12">
              <div className="h-8 w-8 animate-spin rounded-full border-2 border-(--accent) border-t-transparent" />
              <p className="mt-4 text-(--ink-muted)">Importing {selectedTeams.size} teams...</p>
            </div>
          )}
          ...
          {step === "result" && importResult && (
            <div className="space-y-6 text-center">
              ...
              <div className="grid grid-cols-3 gap-4 rounded-lg border border-(--card-stroke) bg-(--card-80) p-4">
                ...
                <div>
                  <div className="text-2xl font-bold text-foreground">{importResult.merged}</div>

...
describe("AreaOverview — summarize + route (no hero/grid duplication)", () => {
	...
	it("sinks empty/unconnected sub-areas to the muted tier, sorted last", () => {
		...
		expect(gridCards.map((c) => c.getAttribute("data-signal-id"))).toEqual([
			"med",
			"gap",
		]);

		const gap = gridCards.find(
			(c) => c.getAttribute("data-signal-id") === "gap",
		)!;
		...
		expect(
			within(gap).getByText(AREA_UNAVAILABLE_EMPTY_STATE.title),
		).toBeInTheDocument();
	});
	...
	it("falls back to the muted tier alone when no real-data signal exists", () => {
		...
		expect(cards.every((c) => c.getAttribute("data-tier") === "muted")).toBe(
			true,
		);
	});
});

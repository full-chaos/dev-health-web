...
describe("ViewSet", () => {
	it("renders vertical view links from nav-visible item data only", () => {
		...
		expect(
			screen.getByRole("link", { name: "Renamed Flow Label" }),
		).toHaveAttribute("href", "/metrics?team=platform");
		expect(
			screen.getByRole("link", { name: "Demoted Route" }),
		).toBeInTheDocument();
	...
	it("renders tabs from nav-visible item data only", () => {
		render(
			<ViewSet
				...
				items={items}
				activeId="flow"
				ariaLabel="Govern views"
			/>,
		);

		expect(
			screen.getByRole("tablist", { name: "Govern views" }),
		).toBeInTheDocument();
		expect(
			screen.getByRole("tab", { name: "Renamed Flow Label" }),
		).toHaveAttribute("href", "/metrics?team=platform");
	...
	it("marks the active vertical item as the current page", () => {
		...
		expect(
			screen.getByRole("link", { name: "Govern Summary" }),

...
export function TeamCategorySankeySection({
	...
	const sankeyMetrics = useMemo(
		() =>
			sankeyFlow
				? computeSankeyMetrics(sankeyFlow.nodes, sankeyFlow.links)
				: null,
		[sankeyFlow],
	);
	const baselineMetrics = useMemo(
		() =>
			baselineFlow
				? computeSankeyMetrics(baselineFlow.nodes, baselineFlow.links)
				: null,
		[baselineFlow],
	);
	...
	const isSingleTeamScope =
		filters.scope.level === "team" && filters.scope.ids.length === 1;
	...
	return (
		<div className="rounded-3xl border border-(--card-stroke) bg-card p-5">
			...
			<div className="mb-4 mt-2 border-l-2 border-(--card-stroke) py-1 pl-3 text-xs leading-relaxed text-(--ink-muted)">
				This view shows where effort appears to land across teams, themes, and
				repos for the selected window. Allocation reflects attribution, not

        ...
        (flow: SankeyResponse | null, topN: number) => {
            ...
            flow.nodes.forEach((node) => {
                ...
                nodeMap.set(normalizedName, {
                    ...
                    itemStyle: existing.itemStyle?.color
                        ? existing.itemStyle
                        : coloredNode.itemStyle,
                });
            });
    ...
    const buildSankeyTooltipFormatter = useCallback(
        (context: {
            ...
            return (params: unknown, unit: string) => {
                ...
                const shareRatio =
                    context.metrics.totalFlow > 0
                        ? resolvedValue / context.metrics.totalFlow
                        : null;
                const clampedShare =
                    shareRatio === null ? null : Math.min(1, Math.max(0, shareRatio));

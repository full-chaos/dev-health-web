...
export function TeamExchangeChordSection({
	...
	const entityCount = useMemo(() => {
		return new Set(
			(records ?? []).flatMap((record) => [record.source, record.target]),
		).size;
	}, [records]);
	...
	const highlightedLabel = useMemo(
		() =>
			dataset?.nodes.find((node) => node.id === highlightedEntity)?.label ??
			null,
			...
			<header className="flex flex-wrap items-start justify-between gap-4">
				<div className="min-w-0">
					...
					<p className="mt-1 text-xs text-(--ink-muted)">
						Exchange of{" "}
						{controls.grouping === "work_type"
							? "work types"
							: `${controls.grouping}s`}{" "}
						across repositories. Shows pairs that frequently touch the same
						work.
					</p>
				</div>
				<ChordChartControls

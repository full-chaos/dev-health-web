...
type InvestmentMix = ReturnType<
	typeof import("@/lib/investmentMix").normalizeInvestmentMix
>;
...
export function InvestmentMixSection({
	...
	const [mixChartType, setMixChartType] =
		useState<TreemapSunburstType>("treemap");
	const [treemapSelection, setTreemapSelection] =
		useState<TreemapSelection | null>(null);
		...
		return buildInvestmentWorkGraphUrl({
			...
			subcategoryKey:
				treemapSelection.type === "subcategory"
					? treemapSelection.subcategoryId
					: null,
		});
		...
		const children = themes.map((theme) => {
				...
				.map((sub, idx) => {
					...
					return {
						...
						itemStyle: {
							...
							opacity:
								typeof subOpacity === "number" ? clamp(subOpacity) : undefined,
						},
						nodeType: "subcategory",
						categoryId: sub.key,
			...
			return {
				...
				itemStyle: {
					color: baseColor,
					opacity:
						typeof themeOpacity === "number" ? clamp(themeOpacity) : undefined,
				},
				nodeType: "theme",
				themeKey: theme.key,
	...
	const formatTreemapTooltip = useCallback(
		(params: unknown, _totalValue: number, unitLabel: string) => {
			...
			const qualityValue =
				typeof data.qualityValue === "number" ? data.qualityValue : null;
			const qualityLabel =
				qualityValue !== null ? formatQuality(qualityValue) : "Unknown";
			const qualityExtra =
				qualityValue !== null

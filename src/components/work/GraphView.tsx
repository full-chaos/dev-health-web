...
function isConnectionSliceId(
	value: string | null,
): value is ConnectionSlice["id"] {
	return Boolean(
		value && CONNECTION_SLICES.some((slice) => slice.id === value),
	);
}

function isInvestmentTheme(
	value: string | null,
): value is (typeof INVESTMENT_THEMES)[number] {
	return Boolean(
		value &&
			INVESTMENT_THEMES.includes(value as (typeof INVESTMENT_THEMES)[number]),
	);
}
...
function isInvestmentSubcategory(
	...
	return Boolean(
		value &&
			INVESTMENT_SUBCATEGORIES.includes(
				value as (typeof INVESTMENT_SUBCATEGORIES)[number],
			),
	);
}
...
function getGraphSearchState(searchParams: URLSearchParams) {
	...
	const subcategory = isInvestmentSubcategory(subcategoryParam)
		? subcategoryParam
		: "all";
	const subcategoryTheme =
		subcategory === "all" ? null : subcategory.split(".", 1)[0];
...
export function GraphView({ filters, activeRole }: GraphViewProps) {
	...
	const searchState = useMemo(
		() => getGraphSearchState(searchParams),
		[searchParams],
	);
	const [selectedNode, setSelectedNode] = useState<SelectedNode | null>(
		searchState.selectedNode,
	);
	const [theme, setTheme] = useState(searchState.theme);
	const [subcategory, setSubcategory] = useState(searchState.subcategory);
	const [connectionSliceId, setConnectionSliceId] = useState(
		searchState.connectionSliceId,
	);
	...
	const activeConnectionSlice =
		CONNECTION_SLICES.find((slice) => slice.id === connectionSliceId) ??
		CONNECTION_SLICES[0];
	const slicedEdges = useMemo(
		() =>
			activeConnectionSlice.edgeTypes.length
				? edges.filter((edge) =>
						activeConnectionSlice.edgeTypes.includes(edge.edgeType),
					)
				: edges,
	...
	const handleNodeClick = useCallback(
		(nodeId: string, nodeType: WorkGraphNodeType) => {
			setSelectedNode((prev) =>
				prev?.id === nodeId && prev?.type === nodeType
					? null
				...
				<div className="bg-card rounded-lg border border-(--card-stroke) p-4">
					...
					{(hiddenEdgeCount > 0 ||
						...
						<div className="mb-4 rounded-xl bg-amber-500/10 px-3 py-2 text-xs text-amber-300">
							Showing {displayEdges.length}{" "}
							{activeConnectionSlice.label.toLowerCase()} edges for browser
							responsiveness
							{slicedEdges.length !== edges.length

...
export function FlowView({ filters, activeRole }: FlowViewProps) {
    ...
    const initialSubTab: FlowSubTab =
        subTabParam && FLOW_TABS.some((t) => t.id === subTabParam)
            ? subTabParam
    ...
    useEffect(() => {
        ...
        import("@/data/devHealthOpsSample")
            ...
            .catch((err: unknown) => {
                console.warn("[FlowView] Failed to load sample hotspot data", err);
            });
    }, []);

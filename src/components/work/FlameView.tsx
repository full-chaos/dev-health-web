      ...
      <section className="rounded-3xl border border-(--card-stroke) bg-card p-6">
        <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            {(["cycle_breakdown", "throughput", "code_hotspots"] as AggregatedFlameMode[]).map(
              (m) => (
                <button
                  key={m}
        ...
        <div className="relative h-[calc(100vh-380px)] min-h-[500px]" data-testid="chart-flame">
          {loading && (
          ...
          {hasData ? (
            <HierarchicalFlameGraph root={flameData.root} unit={flameData.unit} height={600} />
          ) : (
            !loading && (

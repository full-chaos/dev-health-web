...
export function ProductTelemetryDashboard({
	...
	const latestActiveUsers =
		dashboard.dailyActiveUsers.at(-1)?.activeAnonymousUsers;
	const totalRouteEvents = dashboard.topRoutes.reduce(
		(total, row) => total + row.events,
		0,
	);
	const totalFeatureViews = dashboard.featureViews.reduce(
		(total, row) => total + row.views,
		0,
	);
	const totalErrors = dashboard.clientErrors.reduce(
		(total, row) => total + row.errors,
		0,
	);
	...
	return (
		<div className="space-y-6">
			<div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
				{[
					...
					[
						...
						"Page views in top routes",
					],
					[
						"Feature views",
						formatNumber(totalFeatureViews),
						"Stable feature IDs viewed",
					],
					[
						"Client errors",
						formatNumber(totalErrors),
						"Rendered by route and boundary",
					],
			...
			<div className="grid gap-6 xl:grid-cols-2">
				...
				<SectionCard
					title="Feature views"
					description="Stable feature IDs by surface."
				>
					<DataTable

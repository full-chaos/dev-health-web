...
const workOverviewLinks = [
    ...
    },
    {
        id: "flame",
        title: "Flame",
        description: "Icicle-style decomposition for elapsed time, throughput, and hotspots.",
    ...
    },
    {
        id: "graph",
        title: "Work Graph",
        description: "Relationship topology across work, PRs, code, releases, and incidents.",
...
export default async function WorkPage({ searchParams }: WorkPageProps) {
    ...
    if (removedTabRedirect) {
        redirect(buildRemovedWorkTabRedirectTarget(removedTabRedirect, params));
    }
    const activeTab: WorkTab =
        typeof tabParam === "string" && (WORK_TABS as readonly string[]).includes(tabParam)
            ? (tabParam as WorkTab)
            : "overview";
    ...
    const [health, home, wipExplain, blockedExplain, reviewHeatmap, diagnoseSignals] =
        await Promise.all([
            checkApiHealth(),
                            ...
                            {activeTab === "overview" && (
                                <section className="flex flex-col gap-6">
                                    <div className="grid gap-4 lg:grid-cols-3">
                                        ...
                                        <MetricCard
                                            label={
                                                getMetric("wip_saturation")?.label ??
                                                "WIP Saturation"
                                            }
                                            href={buildExploreUrl({
                                        ...
                                        <MetricCard
                                            label={
                                                getMetric("blocked_work")?.label ?? "Blocked Work"
                                            }
                                            href={buildExploreUrl({
                                                metric: "blocked_work",
                                                filters,

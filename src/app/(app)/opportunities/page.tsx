...
export default async function OpportunitiesPage({
	...
	const activeView: ImproveView = IMPROVE_VIEWS.includes(
	...
	const tabs: ReadonlyArray<ModeTabItem<ImproveView>> = [
		...
		},
		{
			...
			href: withFilterParam(
				"/opportunities?view=focus-cards",
				filters,
				activeRole,
			),
		},
				...
				<main className="flex min-w-0 flex-1 flex-col gap-8">
					<header className="flex flex-col gap-4">
						...
						<div>
							...
							<p className="mt-2 text-sm text-(--ink-muted)">
								Capacity planning, AI workflows, and evidence-linked improvement
								opportunities.
							</p>
						</div>
					</header>
					...
					{activeView === "focus-cards" ? (
						...
						<AreaOverview
							...
							description="Improvement opportunities, ordered by severity."
						/>
					)}
				</main>
			</div>

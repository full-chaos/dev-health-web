...
const sectionDescriptions: Record<string, string> = {
	...
	bottleneck:
		"State duration, review latency, and WIP age signals that shape flow.",
		...
		<div className="min-h-screen bg-background text-foreground">
			<div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-6 pb-16 pt-10 md:flex-row">
				...
				<main className="flex min-w-0 flex-1 flex-col gap-8">
					...
					{isMultiTeam ? (
						<SelectedTeamsBadge teamIds={selectedTeamIds} />
					) : null}
					{!review ? (
						<EmptyReviewState
...
async function resolveOperatingReview(
	...
	if (!ceilingReview) {
		return (
			teamReviews.find((review): review is OperatingReview =>
				Boolean(review),
			) ?? null
		);
	}
	...
	return aggregateOperatingReviews({
		ceilingReview,
		reviews: teamReviews.filter((review): review is OperatingReview =>
			Boolean(review),
		),
...
function AllTeamsBadge() {
	return (
		<section className="rounded-2xl border border-(--card-stroke) bg-(--card-80) px-5 py-3 text-xs text-(--ink-muted)">
			...
			<span className="font-medium text-foreground">(All Teams)</span>. Pick a
			team from the <span className="font-medium text-foreground">Team</span>{" "}
			filter above to scope to one or more teams.
		</section>
	);
}
				...
				{review.sections.map((section) => (
					...
					>
						...
						<p className="mt-2 text-xs text-muted-foreground">
							{section.improved.length} improved · {section.worsened.length}{" "}
							worsened · {section.changed.length} changed
						</p>
					</a>
				))}
			</section>
	...
	return (
		...
		>
			<p className="text-sm text-muted-foreground">
				Review these signals as operating patterns, not individual performance.
				Drill into the dedicated AI surfaces when review pressure, quality drag,
				or automation candidates need evidence-level follow-up.
			</p>
	...
	return (
		<div className="rounded-2xl border border-border bg-background/70 p-4">
			<div className="flex items-start justify-between gap-3">
				...
				<span className={statusClass(metric.delta.status)}>
					{metric.delta.status}
				</span>
			</div>
...
function EmptyReviewState({
	...
	return (
		<section className="rounded-[1.75rem] border border-dashed border-border bg-card/70 p-8">
			...
			<p className="mt-2 text-sm text-muted-foreground">
				The surface is ready for {scopeLabel} for week {weekStart}, but no
				backend payload was returned.
			</p>
			<Link
...
function statusClass(status: string): string {
	...
	if (status === "worsened")
		return `${base} bg-rose-500/10 text-rose-700 dark:text-rose-300`;
	if (status === "changed")

...
function riskValue(risk: ThroughputRiskOverlay) {
	if (risk.kind === "review")
		return `${formatNumber(risk.value, { maximumFractionDigits: 1 })}h`;
	if (risk.kind === "wip")
		return `${formatNumber(risk.value, { maximumFractionDigits: 2 })}×`;
	...
	return (
		<div className="rounded-2xl border border-(--card-stroke) bg-(--card-80) p-5">
			...
			<p className="mt-2 text-xs text-(--ink-muted)">
				Threshold{" "}
				{risk.threshold > 0
					? riskValue({ ...risk, value: risk.threshold })
					: "—"}
			</p>
		</div>
	);
	...
	return (
		<section className="rounded-3xl border border-(--card-stroke) bg-(--card-80) p-8">
			...
			<p className="mt-4 text-sm text-(--ink-muted)">
				Not enough throughput history to generate a forecast for this scope. Try
				widening the date range, selecting a different team, or syncing more
				work-item history.
			</p>
		</section>
	);
				...
				<main className="flex min-w-0 flex-1 flex-col gap-8">
					<header className="flex flex-wrap items-center justify-between gap-4">
						<div>
							...
							<p className="mt-2 text-sm text-(--ink-muted)">
								Forecast — not a commitment. Uses rolling 4/8/12-week throughput
								and risk overlays. Backlog and scope are derived from the filter
								bar.
							</p>
						</div>
						<Link
						...
						<>
							<section className="grid gap-4 rounded-3xl border border-(--card-stroke) bg-(--card-80) p-6 md:grid-cols-[auto_1fr]">
								...
								<div className="self-center text-xs text-(--ink-muted) md:text-right">
									Derived from current filter scope — adjust filters above to
									refocus.
								</div>
							</section>

 ...
 * `last_result` field from the backend (verified against
 * IngestTokenResponse in the merged ops source) — status is derived here
 * from `revoked_at`/`expires_at`/`last_used_at` so every list/card in the UI

...
describe("chartUtils", () => {
	describe("formatTooltipValue", () => {
		...
		it("labels absent (undefined) values distinctly from unavailable", () => {
			expect(formatTooltipValue(undefined, "hours")).toBe("No data");
			expect(formatTooltipValue(null, "hours")).toBe("No data");
		});

		it("labels non-finite (NaN/Infinity) values as unavailable", () => {
			expect(formatTooltipValue(NaN, "items")).toBe("Unavailable");
			expect(formatTooltipValue(Infinity, "items")).toBe("Unavailable");
		});
	...
	describe("formatPercent", () => {
		...
		it("labels a zero total as absent (no data to take a share of)", () => {
			expect(formatPercent(10, 0)).toBe("No data");
		});

		it("labels a negative total as absent", () => {
			expect(formatPercent(10, -100)).toBe("No data");
		});

		it("labels non-finite values as unavailable", () => {
			expect(formatPercent(NaN, 100)).toBe("Unavailable");
			expect(formatPercent(100, NaN)).toBe("Unavailable");
			expect(formatPercent(Infinity, 100)).toBe("Unavailable");
		});
	...
	describe("buildPathString", () => {
		it("joins path segments with arrow separator", () => {
			expect(buildPathString(["root", "category", "item"])).toBe(
				"root → category → item",
			);
		});

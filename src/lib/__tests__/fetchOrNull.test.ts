...
describe("fetchOrNull", () => {
  ...
  describe("happy path", () => {
    ...
    it("awaits slow-resolving promises", async () => {
      const slow = new Promise<string>((resolve) => setTimeout(() => resolve("later"), 5));
  ...
  describe("error path", () => {
    ...
    it("returns null for rejection with a string", async () => {
      const result = await fetchOrNull(Promise.reject("string reason"), "string-reject");
      expect(result).toBeNull();
    });

    it("returns null for rejection with undefined / null", async () => {
      expect(await fetchOrNull(Promise.reject(undefined), "undef-reject")).toBeNull();
      expect(await fetchOrNull(Promise.reject(null), "null-reject")).toBeNull();
    });
    ...
    it("resolves (does not reject) when the underlying promise rejects", async () => {
      await expect(fetchOrNull(Promise.reject(new Error("x")), "swallow")).resolves.toBeNull();
    });
  });

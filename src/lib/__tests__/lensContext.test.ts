...
describe("getLensConfig", () => {
    ...
    it.each(["ic", "em", "pm", "leadership"] as const)(
        "returns non-empty investigationOrder for role lens '%s'",
        (role) => {
...
describe("applyLensPriority", () => {
    ...
    it("preserves order when lens is neutral", () => {
        ...
        expect(result.map((i) => i.id)).toEqual(["wip", "review", "churn", "investment", "cycle"]);
    });

...
describe("sortBySeverity", () => {
	...
	it("demotes available low-priority signals below regular available signals", () => {
		const input = [
			signal("flags", "critical", undefined, true),
			signal("quality", "low"),
			signal("coverage", "medium"),
		];
		expect(sortBySeverity(input).map((s) => s.id)).toEqual([
			"coverage",
			"quality",
...
describe("groupByCluster", () => {
	...
	it("sinks unavailable signals to the bottom within their cluster", () => {
		const input = [
			...
			signal("ok", "medium", "Quality"),
		];
		expect(groupByCluster(input)[0].signals.map((s) => s.id)).toEqual([
			"ok",
			"u",
		]);
	});
...
describe("topSignals", () => {
	it("returns the top N available signals by severity, excluding unavailable", () => {
		...
		expect(topSignals(input, 3).map((s) => s.id)).toEqual([
			"crit",
			"high",
			"med",
		]);

...
const nextAuth = NextAuth({
  ...
  providers: [
    ...
    ...(authEnv.AUTH_GOOGLE_ID
      ? [Google({ clientId: authEnv.AUTH_GOOGLE_ID, clientSecret: authEnv.AUTH_GOOGLE_SECRET! })]
      : []),
    ...(authEnv.AUTH_GITLAB_ID
      ? [GitLab({ clientId: authEnv.AUTH_GITLAB_ID, clientSecret: authEnv.AUTH_GITLAB_SECRET! })]
      : []),
  ],
  callbacks: {
    async jwt({ token, user, account, trigger, session }) {
      ...
      if (
        ...
        token.access_token &&
        !user &&
        (!lastImpersonationCheck || now - lastImpersonationCheck > IMPERSONATION_POLL_INTERVAL)
      ) {
        try {
          const backendUrl = getBackendUrl();

...
const investmentPageSource = readFileSync(
	join(appRoot, "investment/page.tsx"),
	"utf8",
);
...
const landscapePageSource = readFileSync(
	join(appRoot, "landscape/page.tsx"),
	"utf8",
);
const bottleneckPageSource = readFileSync(
	join(appRoot, "bottleneck/page.tsx"),
	"utf8",
);
...
const navVisibleDestinations = (): VisibleChildDestination[] =>
	navAreas.flatMap((area) =>
		area.children
			...
			.map((child) => ({ area, child })),
	);

const parseHref = (href: string) => new URL(href, "https://dev-health.local");

const paramValue = (params: URLSearchParams, name: string) =>
...
const expectDeepLinkResolvesToWorkbenchTab = (
	href: string,
	expectedTab: string,
) => {
	...
	expect(tab, `${href} should name its intended Work workbench tab`).toBe(
		expectedTab,
	);
	...
	expect(
		activeView,
		`${href} should not fall through to Diagnose Overview`,
	).toBe("work");
	expect(
...
const mountsPageFilterBar = (source: string) =>
	/<FilterBar(?:Client)?[\s/>]/.test(source);
...
describe("IA preservation invariant #1 — no orphaned views", () => {
	it("keeps every baseline nav/work/deep-link destination reachable", () => {
		const liveReachable = new Set([
			...navVisibleDestinations().map(({ child }) =>
				reachableKeyForHref(child.path),
			),
		...
		const orphaned = baselineEntries.filter(
			(entry) => !liveReachable.has(entry.key),
		);
...
describe("IA preservation invariant #2 — no redirect-only tabs", () => {
	it("backs every navVisible destination with a real route and expected Work resolver branch", () => {
		for (const { area, child } of navVisibleDestinations()) {
			...
			const expectedView =
				child.id === "diagnose-overview" ? "overview" : "work";
	...
	it.each(
		WORK_TABS,
	)("renders Work workbench tab %s instead of a fallback", (tab) => {
		...
		expect(
			workPageSource,
			`${href} should have a concrete page branch`,
		).toContain(`activeTab === "${tab}"`);
	});

	it.each(
		testOpsTabRoutes,
	)("renders TestOps tab $label at $path with real page content", (tab) => {
		const pagePath = join(
			appRoot,
			...tab.path.split("/").filter(Boolean),
			"page.tsx",
		);
		...
		expect(
			source,
			`${tab.path} should render the shared TestOps tab strip`,
		).toContain("<TestOpsTabs");
		expect(
			tabsSource,
			"TestOpsTabs should use the ViewSet primitive",
		).toContain("<ViewSet");
		expect(
			tabsSource,
			"TestOpsTabs should render the ViewSet as tabs",
		).toContain('orientation="tabs"');
		expect(
			tabsSource,
		...
		expect(
			source,
			`${tab.path} should not be a redirect-only tab`,
...
describe("IA preservation invariant #3 — no dead investigation deep-links", () => {
	...
	it.each(
		iaPreservationBaseline.investigationDeepLinks,
	)("resolves $source $intent to its $expectedTab workbench view", (entry) => {
		expectDeepLinkResolvesToWorkbenchTab(entry.href, entry.expectedTab);
	});

	it.each(
		iaPreservationBaseline.directDestinationLinks,
	)("points $source $intent straight at $expectedPath rendering the real $expectedTab content", (entry) => {
	...
	it.each(
		WORK_TABS,
	)("preserves the legacy bare /work?tab=%s deep link (resolver still routes it to Work)", (tab) => {
...
describe("IA preservation invariant #4 — no phantom chrome", () => {
	it("assigns every navVisible route to the area that owns its chrome", () => {
		for (const { area, child } of navVisibleDestinations()) {
			...
			expect(
				selectedAreaIdForPathname(navAreas, pathname),
				routeLabel(area, child),
			).toBe(area.id);
		}
	});
...
describe("IA preservation invariant #5 — one context bar", () => {
	...
	it("checks direct page/layout pairs as one route scope", () => {
		...
		const directories = new Set(
			routeFiles.map((filePath) => dirname(filePath)),
		);
...
describe("IA preservation invariant #7 — reachable redirect aliases stay guarded", () => {
	...
	const routeForPageFile = (filePath: string): string => {
		const segments = relative(appRoot, dirname(filePath))
			.split("/")
			.filter(
				(seg) => seg.length > 0 && !(seg.startsWith("(") && seg.endsWith(")")),
			);
	...
	it("registers every redirect-only route in legacyAliasRoutes (no unguarded aliases)", () => {
		...
		const unguarded = scanRedirectOnlyRoutes(appRoot).filter(
			(route) =>
				!registered.has(route) && !knownNonAliasRedirectPages.has(route),
		);
		expect(
			unguarded,
	...
	it.each(
		iaPreservationBaseline.legacyAliasRoutes,
	)("keeps alias $route redirecting to the live destination $redirectsTo", (alias) => {
		expect(
			routePageExists(alias.route),
			`${alias.route} alias page missing`,
		).toBe(true);
		expect(

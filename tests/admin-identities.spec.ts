...
test("identities page renders empty state with add link", async ({ page }) => {
  ...
  await expect(
    page.getByRole("link", { name: "Add Identity" }).or(page.getByRole("button", { name: "Add Identity" })),
  ).toBeVisible();
});
...
test("add identity button adds provider identity row", async ({ page }) => {
  ...
  await page
    .getByRole("button", { name: /\+\s*Add Identity|Add Identity/i })
    .click();

  await expect(page.locator("select").last()).toBeVisible();
  await expect(page.locator("input[name*='username']").last().or(page.locator("input[placeholder*='username' i]").last())).toBeVisible();
});
...
test("creating identity redirects to list", async ({ page }) => {
  ...
  await page
    .getByRole("button", { name: /Create Identity|Save/i })
    .click();

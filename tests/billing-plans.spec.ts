...
test("superadmin billing plan management supports CRUD and sync", async ({ page }) => {
    ...
    await page
        .locator("textarea[placeholder*='interval']")
        .fill('[{"interval":"monthly","amount":6900,"currency":"usd"}]');
    await page.getByRole("button", { name: "Create plan" }).click();

    await expect(page.getByRole("heading", { name: "Growth" })).toBeVisible();

    const growthCard = page.locator("article", {
    ...
    const updatedCard = page.locator("article", {

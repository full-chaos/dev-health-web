	...
	return {
		...
		sections: [
			...
			},
			{
				...
				worsened: [
					"Review amplification rose for AI-attributed pull requests.",
				],
				changed: [
...
function dispatchGraphQL(
					...
					breakdowns: [
						...
						},
						{
							...
							items: Object.entries(
								investmentMixSample.subcategory_distribution,
	...
	// Capacity forecast.
	if (
		query.includes("capacityForecast") ||
		query.includes("CapacityForecast")
	) {
		return HttpResponse.json({
	...
	if (query.includes("compoundingRisk") || query.includes("CompoundingRisk")) {
		...
		const breakout =
			(variables as { filter?: { breakout?: string } }).filter?.breakout ??
			"REPO";
		const weights = {
			churn: 0.3,
			complexity: 0.3,
	...
	if (query.includes("AIImpactSummary")) {
		return HttpResponse.json({
			data: {
				aiImpactSummary: aiImpactSummaryResponse(
					orgId,
					startDate,
					endDate,
					aiMode,
	...
	if (query.includes("AIRiskBreakdown")) {
		return HttpResponse.json({
			data: {
				aiRiskBreakdown: aiRiskBreakdownResponse(
					orgId,
					startDate,
					endDate,
					aiMode,
	...
	if (query.includes("AIGovernanceSummary")) {
		return HttpResponse.json({
			data: {
				aiGovernanceSummary: aiGovernanceSummaryResponse(
					orgId,
					startDate,
					endDate,
					aiMode,
				...
				aiWorkflowDrilldown: {
					...
					edges: [
						...
						},
						{
							...
							source: "msw",
							evidence:
								"Incident opened inside the post-deploy observation window.",
							provider: "github",
							repoId: "repo:web-app",
						},
					],
...
export const handlers = [
	...
	http.post("*/api/v1/home", () =>
		HttpResponse.json({
			...
			signals: [
				{
					...
					evidence_count: 7,
					why_it_matters:
						"Longer reviews delay delivery and frustrate contributors.",
			...
			limiting_factor: {
				...
				why_it_matters:
					"It is the largest current drag on delivery flow this window.",
				recommended_action:
		...
		HttpResponse.json({
			items: [
				...
				},
				{
					...
					evidence_links: [],
					suggested_experiments: [
						"Quarantine the top 3 flaky specs and track re-runs",
					],
				},
			],
		}),
	...
	http.patch("*/api/v1/admin/sync-configs/:id", async ({ params, request }) => {
		...
		const body = (await request.json()) as Partial<MockSyncConfig> | null;
		const syncConfig = MOCK_SYNC_CONFIGS.find(
			(item) => item.id === syncConfigId,
		);
		if (!syncConfig) {
			return HttpResponse.json(

...
test("/deployments/[deployment_id] renders deployment flame", async ({
  page,
}) => {
  ...
  await expect(
    page.locator("[data-chart-ready='true']").first(),
  ).toBeVisible();
});

test("/deployments/[deployment_id] shows fallback when flame is unavailable", async ({
  page,
}) => {
  ...
  await expect(page.getByRole("heading", { name: "Flame Diagram" })).toBeVisible();
  await expect(
    page.getByText("Flame data unavailable for this deployment."),
  ).toBeVisible();
});

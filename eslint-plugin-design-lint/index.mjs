...
function shouldSkip(context, ruleName, node) {
  ...
  return hasDisableNextLine(sourceCode, ruleName, line) || isAllowlisted(ruleName, filename, line);
}
...
const rules = {
  "no-raw-id-in-jsx": {
    meta: ruleMeta("Disallow raw UUID/hash identifiers in JSX labels.", {
      rawId:
        "Render entity ids through EntityLabel; raw UUID/hash literal '{{value}}' is user-facing.",
    }),
    create(context) {
      function check(node, value) {
        if (!value || !UUID_OR_LONG_HASH_RE.test(value) || /\bUnresolved\b/u.test(value)) return;
        report(context, "no-raw-id-in-jsx", node, "rawId", { value: value.trim().slice(0, 80) });
      }
      return {
        JSXText(node) {
  ...
  "no-hardcoded-style": {
    ...
    create(context) {
      function check(node, value) {
        ...
        if (match)
          report(context, "no-hardcoded-style", node, "hardcodedStyle", { value: match[0] });
      }
      return {
        JSXAttribute(node) {
  ...
  "no-internal-leak": {
    meta: ruleMeta("Disallow internal API, ticket, detector, and telemetry tokens in UI text.", {
    ...
    create(context) {
      function check(node, value) {
        ...
        if (match) report(context, "no-internal-leak", node, "internal", { value: match.label });
      }
      return {
        JSXText(node) {
          check(node, node.value);
      ...
      return {
        CallExpression(node) {
          if (
            ...
            ["toString", "toFixed", "toLocaleString"].includes(node.callee.property.name)
          ) {
            check(node);
          }
          if (node.callee.type === "Identifier" && node.callee.name === "String") check(node);
        },
      };
    },
  },

...
- **A0 Product grammar / ViewSet rule**, a ViewSet is a named item-set that renders EITHER as vertical sidebar children OR as horizontal tabs from a single source; it must NOT render both ways at the same scope unless one route is an explicit aggregate/alias. Navigation grammar: vertical nav = durable destinations under an area; horizontal tabs = sibling views inside one destination; cards = routing aids from summaries/empty states; Lens = perspective applied across surfaces; Reports = shareable/exportable artifacts; 'Dashboard' is not a first-class IA term unless explicitly defined as a saved, configurable composition.

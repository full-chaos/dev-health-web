...
- **A2 Tabs**, sibling views _within_ one destination only. Area children (A1) are destinations, not tabs, and never appear in the sidebar as their tab subviews. Locked child destinations (06-06 canonical — Work Lens removed, Work fully distributed): Diagnose → Overview / Flow / Investment / Landscape / Work Graph / Complexity / Cognitive Load / Bottlenecks / Context Fabric / People / Code; Plan → Overview / Capacity Forecast (Operating Review hidden until real); Improve → Opportunities / Experiments / Automations; Govern → Overview / TestOps / Quality / Delivery Risk / Incident Correlation / Security / Feature Flags / Compounding Risk; AI → Overview / Impact / Review Load / Governance Risk / Automations; Reports → Report Center / Weekly Review / Executive Summary / Export History; Admin → Organization / Connections / Data Confidence / Settings / Billing. Cockpit has no children. Never style a navigation exit as a tab.
- **A2c Provisioned destinations**, Context Fabric is rendered in Diagnose only when the current organization has a valid server-provided `agent_context_runtime` entitlement. Missing, false, invalid, or unavailable entitlement data hides the menu row; the direct route keeps its own not-entitled state and remains the authority for access handling.
- **A2b Canonical horizontal tab lists (ViewSet, `tabs` orientation).** Each destination's in-page tabs follow these fixed lists. **Agents MAY ASSUME a destination renders exactly these horizontal tabs even when a given Penpot board does not draw them explicitly** — the App Shell template carries the canonical rendered example, and Diagnose → Complexity / Investment / Landscape / Cognitive Load are built references. Lists: **Flow** → Overview / Timeline / WIP / Review Flow / Aging Work; **Investment** → Overview / Allocation / Evidence / Confidence (CHAOS-2154: Investment describes effort _allocation paths_, never delivery _flow_ — "flow" is reserved for Metrics/Monitoring. Allocation owns the Sankey/Chord allocation-path visuals, Evidence is the table-first work-unit drilldown, Confidence carries classification confidence + evidence-quality bands + unassigned ownership + rework); **Landscape** → Overview / Teams / Repos / Ownership / Hotspots; **Work Graph** → Overview / Dependencies / Inflow-Outflow / Review Network / Artifacts; **Complexity** → Overview / Flame / Hotspots / Ownership Risk / Churn; **Cognitive Load** → Overview / Heatmap / Context Switching / Focus Pressure / Load Drivers; **Govern → TestOps** → Overview / Pipelines / Tests / Coverage. Destinations not listed (Overview, People, Code, Bottlenecks) have no horizontal tabs. Heatmap folds into Cognitive Load; Flame folds into Complexity. Evidence is a cross-cutting drawer/rail elsewhere, but Investment owns a dedicated **Evidence** tab (its table-first work-unit drilldown).

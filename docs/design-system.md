...
- **A8 No internal/impl leakage**, never render in customer copy: raw IDs, API/GraphQL paths (`/api/v1/...`, `api/graphql?query=...&scope_id=...`), graph edge names (`DEPLOYS`, `LINKED_INCIDENT`), detector/telemetry jargon, version tags (`V1 SPARKLINE`), or Linear IDs (`CHAOS-1757`). Dev-only details go behind debug mode. Remove `Debug Filters` from customer/explore views.
